...
def analyze_stock(data, criteria_config):
    ...
    if potential_rs >= 30 and potential_pct >= 7:
        ...
        score += 10
        criteria.append(f'⚠️ Potential: Good (₹{potential_rs:.0f}, {potential_pct:.1f}%) [10 pts]')
    else:
    ...
    if macd > 10:
        ...
        score += 10
        criteria.append(f'⚠️ MACD: Slightly Bullish ({macd:.1f}) [10 pts]')
    elif macd > -3:
        score += 5
        criteria.append(f'⚠️ MACD: Turning ({macd:.1f}) [5 pts]')
    else:
        score += 0
    ...
    if 15 <= bb <= 30:
        ...
        score += 8
        criteria.append(f'⚠️ BB: Middle ({bb:.0f}%) [8 pts]')
    else:
        score += 0
    ...
    if vol >= 3.0:
        ...
        score += 8
        criteria.append(f'⚠️ Volume: Above Average ({vol:.1f}x) [8 pts]')
    else:
        score += 0
    ...
    if change >= 5:
        ...
        score += 2
        criteria.append(f'⚠️ Daily: Slight gain ({change:+.1f}%) [2 pts]')
    else:
        score += 0
...
if st.session_state.scan_results:
    ...
    col6.metric("⚠️ Watchlist (55-64)", len(watchlist))
...
<p style='font-size:0.85rem;'>⚠️ Educational purposes only. Not financial advice.</p>
</div>
